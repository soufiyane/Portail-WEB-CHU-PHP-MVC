...
global $i;
	    ...
	    // si la longueur du contenu de la variable est supérieur à 0
			...
			$queryString1=substr($queryString, 0, $pos);
			...
			if(strlen($queryString1) >0)
			{
				...
				if($result)
				{
                  ...
                  //    echo '<li href="http://localhost/framework/public/js/historique_cfps.php?id=cfps.01'.$id.'"  target="popupModal2" title="This is title"
		         	//	onClick="fill(this,\''.$id.' | '.$identi.' | '.$nom.' '.$prenom.'\');">'.$identi1.'&nbsp|&nbsp'.$nom1.'&nbsp'.$prenom1.'</li>';

                       //echo '<li> aaaa </li>';
                       echo '<li onClick="fill1(\''.$lib.' | '.$id.'\'); document.getElementById(\'listesolu'. $i.'\').value='.$id.';
                       "  onMouseOver="voirdiv1('.$id.')" onMouseOut="cacherdiv()">'.$lib1.  '</li>';
                      




                       // ajouter \' dans fill1 pour que le parametre soit une chaine de caractere khra
