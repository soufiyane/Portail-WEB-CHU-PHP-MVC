<?php	
	...
	require_once('includes/alias_tables.php');
		...
		if(isset($_POST['queryString']))
		{
			...
			{
				$result = mysqli_query($db,"SELECT matricule,nom, prenom, matricule, type_casq FROM $tableagent WHERE nom LIKE '$queryString%' OR prenom LIKE '$queryString%' OR matricule LIKE '$queryString%' ORDER BY nom ASC");
				if($result)
				{
					...
					while ($data = mysqli_fetch_object($result))
					{
		         		...
		         		echo '<li onClick="fill2(\''.$id.' | '.$identi.' | '.$nom.' '.$prenom.'\');">'.$identi1.'&nbsp|&nbsp'.$nom1.'&nbsp'.$prenom1.$casque.'</li>';
	         		}
				}
				else
				{
