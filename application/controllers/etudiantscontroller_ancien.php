...
curl_close ($ch);
if ($error_no == 0) {
    $string = 'File downloaded succesfully.';
} else {
    $sting = 'File download error:' . $error_msg . ' | ' . $error_no;
}
fclose($file);
...
//// TENTATIVE DE TELECHARGEMENT DU FICHIER SUR LE SERVEUR ET ENREGISTREMENT EN LOCAL
if (ftp_get($conn_id, $destination, $source, FTP_BINARY)) {
   $string.="Fichier distant enregistré avec succès dans $destination"."<br>";
   $string.="<br><br><br>ATTENTION !!! vous vous apprêtez à importer le fichier dans la base de données <b>depuis le serveur ftp</b>, Validez vous cette action ?<br>
    ...
    $string.="Il y a eu un problème avec la copie du fichier depuis le serveur ftp"."<br>";
}
ftp_close($conn_id);
$this->set('string',$string);
}
...
//MAJ de l'étudiant
if(mysqli_num_rows($this->_template->get('req_etudiant') )!= 0){ // si l'etudiant existe ds la table etudiants
    ...
    //**************************************************************************************
    	  ...
    	  $this->Etudiant->query("UPDATE etudiants SET annee='$annee', classe = '$classe',  mail = '$mail',  tel = '$telephone',  pp = '$pp',  date_maj = curdate() where matricule = ".$matricule."");

        
        if($pp==1 || $pp==2){$this->Etudiant->query("UPDATE etudiants SET carte='PRO' where matricule = ".$matricule."");}
...
//echo $query;
 ...
 /* if (isset($_GET['ftp'])) {
     ...
     $destination_photo_jpg= $destination_photos.'\\'.$matricule.'.jpg';
     $destination_photo_jpg = str_replace('\\', '/', $destination_photo_jpg);
     //echo $destination_photo_jpg;


  

 	   $source_photo_jpeg=$source_photos.$matricule.'.jpeg';

     $destination_photo_jpeg= $destination_photos.'\\'.$matricule.'.jpeg';
     $destination_photo_jpeg = str_replace('\\', '/', $destination_photo_jpeg);
      if (@ftp_get($conn_id,  $destination_photo_jpg,  $source_photo_jpg, FTP_BINARY)) {

       	 $photo='photos/'.$matricule.'.jpg';
//************************************************************
         $this->Etudiant->query("UPDATE etudiantss SET photo = '$photo'  where matricule = ".$matricule."");	
         
          $message.= "L'image pour l'étudiant" .$nom." ".$prenom." a été copié avec succée..."."<br>";
          
          } else 
          if (@ftp_get($conn_id,  $destination_photo_jpeg, $source_photo_jpeg, FTP_BINARY)) {
...
//*************************************************************************
         $this->Etudiant->query("UPDATE etudiantss SET photo = '$photo'  where matricule = ".$matricule."");	
          ...
          $message.= "Il y a eu un problème lors de La copie de l'image pour l'étudiant ".$nom." ".$prenom."<br>";
          
          $message.= "La copie de l'image pour l'étudiant " .$nom." ".$prenom."  a échoué..."."<br>";
